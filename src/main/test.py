...
class BinomialTree:
    ...
    def __init__(self, value):
        "Create a one-node searchTree. value is the priority of this node"
        self.value = value
        self.rank = 0
        self.children = []
...
class BinomialQueue:
    ...
    def __str__(self):
        s = """elements: %d min: %s
        min_tree_rank: %d
        searchTree vector: """ % (self.elements, str(self.min), self.min_tree_rank)
...
if __name__ == "__main__":
    ...
    for t in dic_test.keys():
        print(dic_test[t][0:2])